...
const Text: React.FC<{ productType: string; Category: string; lang: string }> = ({
  ...
  const keyLang = lang as keyof Lang; // Sélectionne la clé de la langue dynamique

  if (productType === "shoe") {
    return (
      <>
        <div className={`shoe-category card ${Category === "classic" ? Category : "d-none"} ${isRtl}`}>
          <h2 className="category-title">{translations[keyLang]?.classic?.title}</h2>
          <ul className="category-description">
            {translations[keyLang]?.classic?.description.map((desc, index) => (
              <li key={index}>{desc}</li>
            ))}
          </ul>
        </div>
        <div className={`shoe-category card ${Category === "basket" ? Category : "d-none"} ${isRtl}`}>
          <h2 className="category-title">{translations[keyLang]?.basket?.title}</h2>
          <ul className="category-description">
            {translations[keyLang]?.basket?.description.map((desc, index) => (
              <li key={index}>{desc}</li>
            ))}
          </ul>
        </div>
        <div className={`shoe-category card ${Category === "medical" ? Category : "d-none"} ${isRtl}`}>
          <h2 className="category-title">{translations[keyLang]?.medical?.title}</h2>
          <ul className="category-description">
            {translations[keyLang]?.medical?.description.map((desc, index) => (
              <li key={index}>{desc}</li>
            ))}
          </ul>
        </div>
        <div className={`shoe-category shadow card ${Category === "mocassin" ? Category : "d-none"} ${isRtl}`}>
          <h2 className="category-title">{translations[keyLang]?.mocassin?.title}</h2>
          <ul className="category-description">
            {translations[keyLang]?.mocassin?.description.map((desc, index) => (
              <li key={index}>{desc}</li>
            ))}
          </ul>
        </div>
  ...
  return null; // Retourne null si le type de produit n'est pas "shoe"
};
