...
import usePantsData from "../Server/pantsData.tsx";
...
const Pants: React.FC = () => {
  ...
  useEffect(() => {
    const show = () =>{
        const filtredStreamedShoes = filterData(pantsData, selectedCriteria);
        setFilteredProduct(filtredStreamedShoes); 
      };
      show();
  }, [selectedCriteria, pantsData]);
  ...
  const handleProductSearch = (criterias: DataToFilter) => {
    ...
    const filteredShoes = filterData(pantsData, criterias);
  ...
  return (<>
    ...
    <div className="" style={{width:'100%', height:"15rem"}}>
      <img src={pant} alt="" style={{width:'100%', height:'100%'}}/>
    </div>
    ...
    {pantsData.length>0?<Products pData={filteredProduct} 
              pDataDetails={pantsDataDetails} 
              productShowed="Pant" 
